...
# determine the min/max y value for Sub_metering across the 3 datasets for ylim
miny <- min(act_wkday$steps,act_weday$steps)
maxy <- max(act_wkday$steps,act_weday$steps)

# draw the empty plot base
plot(x=act_wkday$interval,y=act_wkday$steps,
     xlab="Steps", 
     xaxis="n",
     ylim=c(miny,maxy),
     ylab="WeekDay Interval", 
     main="Differences in Mean Activity Patterns between Weekdays and Weekends"
     ) 

# add the lines to the plot
lines(x=act_wkday$interval, act_wkday$steps, type="l", col="blue")

par(mar=c(4,4,1,1))

# draw the empty plot base
plot(x=act_weday$interval,y=act_weday$steps,
     xlab="Steps", 
     ylim=c(miny,maxy),
     ylab="WeekEnd Interval", 
     ) 

# add the lines to the plot
lines(x=act_weday$interval, act_weday$steps, type="l", col="red")
